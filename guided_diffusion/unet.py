...
class UNetModel(nn.Module):
    ...
    def __init__(
        ...
        self.previous_time_idx_in_list = -1
        self.previous_sigma = -1
    ...
    def forward_10102024(self, x, timesteps, y=None):
        """
    ...
    def forward(self, x, sigma, y=None):
        ...
        :return: an [N x C x ...] Tensor of outputs.
        """
        if self.previous_sigma == -1:
            self.previous_sigma = sigma
        else:
            # This condition is to initialize parameters when current PnP iteration is done and consider next image
            if self.previous_sigma < sigma:
                self.initialize_self_parameters()
                self.previous_sigma = sigma
            else:
                self.previous_sigma = sigma
        assert (y is not None) == (
        ...
        log_gradient_x_i = - th.sqrt((1)/(1-alphas)) * model_output # * By equation 39 in overleaf
        log_gradient_x_i = log_gradient_x_i
        
        # General score-based denoising template
        c = th.sqrt(alphas)
        sigma_square = ((1-alphas)/(alphas))
        x_hat_MMSE = x + c*sigma_square*log_gradient_x_i

        return x_hat_MMSE

    # def potential(self, x, time_corresponding_sigma_denoiser, alphas_corresponding_sigma_denoiser):
    def potential(self, x, sigma, *args, **kwargs):
        self.student_grad = StudentGrad(self)
        N = self.student_grad(x, sigma = sigma)
        # print(f"self.alpha: {self.alpha}")
        # raise ValueError("potential")
        return (
            0.5
            * 1 
            * th.norm((x - N).view(x.shape[0], -1), p=2, dim=-1) ** 2
        )
    def initialize_self_parameters(self):
        self.previous_time_idx_in_list = -1
        self.previous_sigma = -1
...
class StudentGrad1(nn.Module):
    def __init__(self, denoiser):
        super().__init__()
        self.model = denoiser

    def forward(self, x, time_corresponding_sigma_denoiser, alphas_corresponding_sigma_denoiser):
        model_dictionary = p_mean_variance(model = self.model, x = th.sqrt(alphas_corresponding_sigma_denoiser) * (x), t = time_corresponding_sigma_denoiser, alphas= alphas_corresponding_sigma_denoiser)
        # model_dictionary = p_mean_variance(model = self.denoiser, x = torch.sqrt(self.alphas_corresponding_sigma_denoiser) * (x), t = time_corresponding_sigma_denoiser, alphas = self.alphas_corresponding_sigma_denoiser)
        N = model_dictionary['x_hat_MMSE']
        # return self.model(x, sigma)
        return N
