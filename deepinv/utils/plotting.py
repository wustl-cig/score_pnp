...
from util.tweedie_utility import clear_color

# ! HERE I newly add the line
...
def prepare_images(x, y, x_net, x_nl=None, rescale_mode="min_max"):
    ...
    """
    # ! HERE, I newly added this normalization code ---------------------------------------------
    x = clear_color(x)
    # print(f"x.shape: {x.shape}")
    y = clear_color(y)
    # print(f"y.shape: {y.shape}")
    if x_nl is not None:
        x_nl = clear_color(x_nl)
    # print(f"x_nl.shape: {x_nl.shape}")
    x_net = clear_color(x_net)
    ...
    with torch.no_grad():
        ...
        print(f"vis_array.shape: {vis_array.shape}")
        # * vis_array.shape: torch.Size([12, 3, 128, 128]
...
def plot_curves(metrics, save_dir=None, show=True, title=None):
    r"""
    ...
    # ! HERE I newly add one line below because of Latex error
    ...
    for i, metric_name in enumerate(metrics.keys()):
        ...
        if len(metric_val) > 0:
            ...
            if metric_name == "cost":
                metric_val[batch_size-1] = [x/metric_val[batch_size-1][0] for x in metric_val[batch_size-1]] 
                # print(f"[plotting.py plot_curves: metric_name: {metric_name}] metric_val[{batch_size}]: {(metric_val[batch_size-1])/metric_val[batch_size-1][0]}")
            ...
            numpy_metric_array = np.array(metric_val[batch_size-1])
            # print(f"len(numpy_metric_array): {len(numpy_metric_array)}")
            title_npy = f"{metric_name}_{title}.npy"
            np.save(save_dir / title_npy, numpy_metric_array)
                    
            # print(f"[plotting.py plot_curves: metric_name: {metric_name}] metric_val[{batch_size}]: {metric_val[batch_size-1]}")
            # print(f"i: {i}/batchsize: {batch_size}")
    ...
    if save_dir:
        plt.savefig(save_dir / title_png)
    if show:
        plt.show()
