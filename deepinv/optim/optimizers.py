...
class BaseOptim(nn.Module):
    ...
    def init_metrics_fn(self, X_init, x_gt=None):
        ...
        # psnr = [[] for i in range(self.batch_size)]
        # init["psnr"] = psnr
        init["psnr"] = [[] for i in range(self.batch_size)]
        # init["psnr"] = []
        # if self.has_cost:
        ...
        # raise ValueError(f"init['psnr'][0]: {len(init['psnr'][0])}\ninit['residual'] = {len(init['residual'][0])}")
        # print(f"len(psnr): {len(psnr)}")
        return init
    ...
    def update_metrics_fn(self, metrics, X_prev, X, x_gt=None):
        ...
        """
        # print(f"beginning metrics['psnr'][0]: {len(metrics['psnr'][0])}")
        # print(f"beginning metrics['residual'][0]: {len(metrics['residual'][0])}")
        if metrics is not None:
            ...
            for i in range(self.batch_size):
                ...
                residual2 = (
                    # ((x_prev[i] - x[i]).norm() / (x[i].norm() + 1e-06))
                    (((x_prev[i] - x[i]).norm(p=1))/((x[i]).norm(p=1)))
                    .detach()
                    .cpu()
                    .item()
                )
                # residual = 1
                metrics["residual"][i].append(residual1)
                psnr = cal_psnr(x[i], x_gt[i])
                metrics["psnr"][i].append(psnr)
                metrics["cost"][i].append(residual2)
                # if x_gt is not None:
                #     psnr = cal_psnr(x[i], x_gt[i])
                #     metrics["psnr"][i].append(psnr)
                #     # print("", end="")
                # raise ValueError(f"self.has_cost: {self.has_cost}")
                # if self.has_cost:
                #     # psnr = cal_psnr(x[i], x_gt[i])
                #     # metrics["psnr"][i].append(psnr)
                #     F = X["cost"][i]
            ...
            # print(f"metrics['psnr'][0]: {len(metrics['psnr'][0])}")
            # print(f"metrics['residual'][0]: {len(metrics['residual'][0])}")
        return metrics
...
def create_iterator(iteration, prior=None, F_fn=None, g_first=False):
    ...
    # raise ValueError(f"explicit_prior: {explicit_prior}")
