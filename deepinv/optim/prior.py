...
class PnP(Prior):
    ...
    # ! HERE I change input argument
    # def __init__(self, denoiser, *args, **kwargs):
    def __init__(self, denoiser, is_diffusion_model = False, diffusion_model_type = "cnn", diffusion_config=None, device = None, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.denoiser = denoiser
        self.explicit_prior = False
        # ! HERE I add two variables below
        self.is_diffusion_model = is_diffusion_model
    ...
    def prox(self, x, sigma_denoiser, *args, **kwargs):
        ...
        """
        # if self.is_diffusion_model == True:
        #     # TODO: time_corresponding_sigma_denoiser
        #     if self.diffusion_model_type == "vp_score":
        #         time_corresponding_sigma_denoiser, alphas_corresponding_sigma_denoiser = get_noiselevel_alphas_timestep(sigma_denoiser, beta_at_clean=self.diffusion_config["vp_diffusion"]["beta_at_clean"], denoiser_network_type = self.diffusion_model_type, num_diffusion_timesteps = 1000, last_time_step = 0)
        #         time_corresponding_sigma_denoiser = torch.tensor([time_corresponding_sigma_denoiser] * x.shape[0], device=self.device)
        ...
        #         model_dictionary = p_mean_variance(model = self.denoiser, x = torch.sqrt(alphas_corresponding_sigma_denoiser) * (x), t = time_corresponding_sigma_denoiser, alphas = alphas_corresponding_sigma_denoiser)
        #         model_output = model_dictionary['x_hat_MMSE']
        #         # model_output = self.denoiser(x, time_corresponding_sigma_denoiser)
...
class RED(Prior):
    ...
    # def __init__(self, denoiser, *args, **kwargs):
    # ! HERE I modify input argument
    def __init__(self, denoiser, score=None, is_diffusion_model = False, diffusion_model_type = "cnn", diffusion_config=None, device = None, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.denoiser = denoiser
        self.explicit_prior = False
        # ! HERE I add two variables below
    ...
    def grad(self, x, sigma_denoiser, *args, **kwargs):
        ...
        # ! HERE I MODIFY WITH THE CODE BELOW.
        # if self.is_diffusion_model == True:
        #     # TODO: time_corresponding_sigma_denoiser
        #     if self.diffusion_model_type == "vp_score":
        #         time_corresponding_sigma_denoiser, alphas_corresponding_sigma_denoiser = get_noiselevel_alphas_timestep(sigma_denoiser, beta_at_clean=self.diffusion_config["vp_diffusion"]["beta_at_clean"], denoiser_network_type = self.diffusion_model_type, num_diffusion_timesteps = 1000, last_time_step = 0)
        ...
        # else:
        # return self.denoiser(x, sigma_denoiser)
        residue = x - self.denoiser(x, sigma_denoiser)
        # raise ValueError()
        return residue
